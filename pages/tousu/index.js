...
Page({
  ...
  picupload: function (tempfile, filename, img_id) {
    ...
    wx.uploadFile({
      url: app.globalData.host + "/wx/company_tousu_pic",
      formData: {
        'field_name': filename,
...
//提交投诉
  tousu:function(){
    ...
    handlogin.isLogin(() => {
      wx.request({
        url: app.globalData.host + '/wx/tousu',
        data: {
          'user_id': wx.getStorageSync('user_id'),
          's_id': wx.getStorageSync('s_id'),
          'order_id': _this.data.order_id,
          'phone':wx.getStorageSync('phone'),
          'tousu_to_driver': JSON.stringify(tousu_to_driver)
        },
        method: 'POST',
        header: {
        ...
        success: function (res) {
          ...
          if (!res.data.err && res.data.ok=='1') {
            ...
            handlogin.handError(res, _this.tousu)
          }
        },
        fail: function (res) {
          wx.showToast({
